/**
 * Faixas oficiais da Rubrica Blueprint IA (16 dimensões, níveis 1–5).
 * Fonte: Rubrica_16_Dimensoes_Niveis_1a5.xlsx e Guias de Progressão D01–D16.
 */
...
/** Rótulos usados nos books e relatórios executivos (Blueprint operacional). */
...
/** Converte score contínuo (1.0–5.0) em nível inteiro 1–5 conforme rubrica. */
...
export function blocoFaixasRubricaMarkdown() {
  return `## Faixas de nível (Rubrica Blueprint IA)

| Nível | Nome | Faixa de score |
|:---:|:---|:---:|
${RUBRICA_FAIXAS_NIVEL.map(
  (f) => `| **${f.nivel}** | ${f.nome} | ${f.min.toFixed(1)} – ${f.max.toFixed(1)} |`
).join('\n')}

- **Agregação:** o score geral é a média simples das dimensões com score > 0.
- **MIT CISR:** o modelo público descreve **quatro estágios empresariais**; a escala 1–5 acima é operacional Blueprint IA, mapeável aos estágios MIT quando pertinente.`;
}
